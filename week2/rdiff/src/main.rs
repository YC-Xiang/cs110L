        ...
        for j in 0..n {
            if seq1[i] == seq2[j] {
                ...
                grid.set(
                    i + 1,
                    j + 1,
                    std::cmp::max(grid.get(i + 1, j).unwrap(), grid.get(i, j + 1).unwrap()),
                )
                .unwrap();
            }
        }
...
fn print_diff(lcs_table: &Grid, lines1: &Vec<String>, lines2: &Vec<String>, i: usize, j: usize) {
    ...
    if i > 0 && j > 0 && x[i-1] == y[j-1] {
        print_diff(lcs_table, x, y, i-1, j-1);
        println!("  {}", x[i-1]);
    } else if j > 0 && (i == 0 || c.get(i,j-1) >= c.get(i-1, j)) {
        print_diff(lcs_table, x, y, i, j-1);
        println!("> {}", y[j-1]);
    } else if i > 0 && (j == 0|| c.get(i,j-1) < c.get(i-1,j)) {
        print_diff(lcs_table, x, y, i-1, j);
        println!("< {}", x[i-1]);
    } else {
        println!("");
    }
...
fn main() {
    ...
    let x = read_file_lines(filename1).expect("read first file error");
    let y = read_file_lines(filename2).expect("read second file error");
