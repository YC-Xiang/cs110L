...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        let path0 = format!("/proc/{pid}/fd/{fd}");
        let path1 = format!("/proc/{pid}/fdinfo/{fd}");

        let link0 = fs::read_link(path0).ok()?;

        let name = OpenFile::path_to_name(link0.to_str()?);
        let content = fs::read_to_string(path1).ok()?;
